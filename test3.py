...
import panel_data_preparer
import importlib
importlib.reload(panel_data_preparer)  # Ensure the latest version of PanelDataPreparer is used
from panel_data_preparer import PanelDataPreparer
...
def get_models_and_param_grids(use_random_search=False, n_iter_random_search=10):
    ...
    if use_random_search:
        param_grids = {
            ...
            "DecisionTree": {
                'max_depth': [None] + list(randint(3, 21).rvs(size=min(5, n_iter_random_search // 2), random_state=42)),
...
# Configuration dictionary for PanelDataPreparer
# Values that were previously global variables specific to data prep are now defined here.


data_preparer = PanelDataPreparer(initial_df=data_precleaning, config=c)
...
def train_evaluate_model(model_name_key, model_instance, X_train, y_train, X_test, y_test, 
                         param_grid=None, cv_splitter=None, # type: ignore
    ...
    if param_grid and cv_splitter: 
        ...
        min_samples_for_cv = cv_splitter.get_n_splits() * 2 
        ...
        elif len(np.unique(y_train)) == 1 and model_name_key not in ["DummyRegressor", "DecisionTree", "XGBoost"]: 
    ...
    try:
        ...
        if not X_train.empty and len(X_train) > 0: 
    ...
    mae = mean_absolute_error(y_test, predictions_test) # type: ignore
    ...
    if len(predictions_train) > 0 and len(predictions_train) == len(y_train) and not y_train.empty:
        try:
            rmse_train = np.sqrt(mean_squared_error(y_train, predictions_train)) # type: ignore
        except Exception as e_train_rmse:
            print(f"    Could not calculate train RMSE for {model_name_key}: {e_train_rmse}")
            rmse_train = np.nan
    elif not y_train.empty and len(predictions_train) == 0 : 
...
def train_evaluate_ensemble(trained_models_dict, X_train_scaled, y_train, X_test_scaled, y_test):
    ...
    estimators = []
    potential_base_models = ["XGBoost", "Ridge", "ElasticNet"] 
           ...
           trained_models_dict[model_name_key].get('model_object') is not None:
            ...
            try: 
                if hasattr(base_model_obj, "coef_") or hasattr(base_model_obj, "feature_importances_") or \
                   (isinstance(base_model_obj, XGBRegressor) and base_model_obj._Booster is not None): 
    ...
    try:
        ...
        if not X_train_scaled.empty and len(X_train_scaled) > 0:
            ...
            if len(predictions_train) == len(y_train): # type: ignore
...
for horizon_val in FORECAST_HORIZONS:
    ...
    # Prepare data for the current horizon using PanelDataPreparer
    ...
    if SAVE_ARTIFACTS:
        ...
        if y_test is not None and not y_test.empty: # type: ignore
        ...
        if X_train_orig is not None and not X_train_orig.empty: # type: ignore
...
#--------------------------------------------------------------------------------------------------------------------
# Keep your plotting functions (`plot_features_timeseries_flat`, etc.)
# as they are in your original file. For brevity, I'm not repeating them here.
# Ensure they are defined in this script.
# The call to plot_features_timeseries_flat should use `all_data_prepared`.
